...
angular.module('ngCart', ['ngCart.directives', 'ngSanitize'])
    ...
    .service('ngCart', ['$rootScope', 'ngCartItem', 'ngCartBox', 'store', '$sce', function ($rootScope, ngCartItem, ngCartBox, store, $sce) {
        ...
        this.addItem = function (id, name, price, weight, size, quantity, category, categoryname, categoryquantity, data) {
            ...
            if (typeof inCart === 'object'){
                ...
                var newItem = new ngCartItem(id, name, price, weight, size, quantity, category, categoryname, categoryquantity, data);
                this.$cart.items.push(newItem);
                //console.log('add item categoryname:'+categoryname);
                if(typeof(this.$cart.categorizeitems[categoryname]) == "undefined") {
                    this.$cart.categorizeitems.push(categoryname);
                    this.$cart.categorizeitems[categoryname] = [];
                    this.$cart.categorizeitems[categoryname].push(newItem);
                    //console.log(this.$cart.categorizeitems);
                } else {
                    //console.log(this.$cart.categorizeitems);
                    this.$cart.categorizeitems[categoryname].push(newItem);
                    //console.log('category variable already created !');
                }
        ...
        this.addBox = function (id, name, price, weight, size, quantity, category, categoryname, categoryquantity, data) {
            ...
            if (typeof inCart === 'object'){
                ...
                var newBox = new ngCartBox(id, name, price, weight, size, quantity, category, categoryname, categoryquantity, data);
                this.$cart.boxes.push(newBox);
                //console.log('add box categoryname:'+categoryname);
                if(typeof(this.$cart.categorizeitems[categoryname]) == "undefined") {
                    this.$cart.categorizeitems.push(categoryname);
                    this.$cart.categorizeitems[categoryname] = [];
                    this.$cart.categorizeitems[categoryname].push(newBox);
                    //console.log(this.$cart.categorizeitems);
                } else {
                    //console.log(this.$cart.categorizeitems);
                    this.$cart.categorizeitems[categoryname].push(newBox);
        ...
        this.getCategorizeItems = function(){
            //console.log(this.getCart().categorizeitems);
            return this.getCart().categorizeitems;
        };

        this.getCategorizeItemsHtml = function(){
            var categorizeItems = this.getCategorizeItems();
            console.log(categorizeItems);
            //var items = this.getItems();
            //console.log(items);
            var categorizeItemsHtml = '';
            for ( var i = 0, l = categorizeItems.length; i < l; i++ ) {
                console.log(categorizeItems[i]);
                //categorizeItemsHtml += '<style>.main-categories-list{margin-left:0px !important;}</style>';
                categorizeItemsHtml += '<ul class="main-categories-list" style="margin-left:0px !important;">';
                categorizeItemsHtml += '<li style="color: #f38200;font-size: 18px;font-weight: bold;line-height: 25px;">'+categorizeItems[i]+'<ul>';
                    console.log(categorizeItems[categorizeItems[i]]);
                    angular.forEach(categorizeItems[categorizeItems[i]], function(catitemk, catitemv) {
                        console.log('catitemk:');
                        console.log(catitemk.getName());
                        categorizeItemsHtml += '<li style="color: #042b3b;font-size: 15px;line-height: 18px;"><b>'+catitemk.getQuantity()+'</b>&nbsp;&nbsp;&nbsp;'+catitemk.getName()+'</li>';
                        //console.log('catitemv:'+catitemv);
                    });
                categorizeItemsHtml += '</ul></li>';
                categorizeItemsHtml += '</ul>';
            }
            return $sce.trustAsHtml(categorizeItemsHtml);
            //return '<div class="test" style="border:1px solid red;">sdfhybjsfbh dsjfbhdshf sdjfbhdfb</div>';
        };
        ...
        this.toObject = function() {
            ...
            if (this.getItems().length === 0 && this.getBoxes().length === 0) return false;

            var items = [];
            var categorizeitems = [];
            angular.forEach(this.getItems(), function(item){
                items.push (item.toObject());
                //console.log('add item categoryname:'+item.getCategoryName());
                /*if(typeof(this.$cart.categorizeitems[categoryname]) == "undefined") {
                    this.$cart.categorizeitems[categoryname] = [];
                    this.$cart.categorizeitems[categoryname].push(newItem);
                    console.log(this.$cart.categorizeitems);
        ...
        this.$restore = function(storedCart){
            ...
            angular.forEach(storedCart.items, function (item) {
                _self.$cart.items.push(new ngCartItem(item._id, item._name, item._price, item._weight, item._size, item._quantity, item._category, item._categoryname, item._categoryquantity, item._data));
                //console.log('add item categoryname:'+item._categoryname);
                if(typeof(_self.$cart.categorizeitems[item._categoryname]) == "undefined") {
                    _self.$cart.categorizeitems.push(item._categoryname);
                    _self.$cart.categorizeitems[item._categoryname] = [];
                    _self.$cart.categorizeitems[item._categoryname].push(new ngCartItem(item._id, item._name, item._price, item._weight, item._size, item._quantity, item._category, item._categoryname, item._categoryquantity, item._data));
                    //console.log(_self.$cart.categorizeitems);
                } else {
                    //console.log(_self.$cart.categorizeitems);
                    _self.$cart.categorizeitems[item._categoryname].push(new ngCartItem(item._id, item._name, item._price, item._weight, item._size, item._quantity, item._category, item._categoryname, item._categoryquantity, item._data));
                    //console.log('category variable already created !');
            ...
            angular.forEach(storedCart.boxes, function (box) {
                _self.$cart.boxes.push(new ngCartBox(box._id, box._name, box._price, box._weight, box._size, box._quantity, box._category, box._categoryname, box._categoryquantity, box._data));
                if(typeof(_self.$cart.categorizeitems[box._categoryname]) == "undefined") {
                    _self.$cart.categorizeitems.push(box._categoryname);
                    _self.$cart.categorizeitems[box._categoryname] = [];
                    _self.$cart.categorizeitems[box._categoryname].push(new ngCartBox(box._id, box._name, box._price, box._weight, box._size, box._quantity, box._category, box._categoryname, box._categoryquantity, box._data));
                    //console.log(_self.$cart.categorizeitems);
                } else {
                    //console.log(_self.$cart.categorizeitems);
                    _self.$cart.categorizeitems[box._categoryname].push(new ngCartBox(box._id, box._name, box._price, box._weight, box._size, box._quantity, box._category, box._categoryname, box._categoryquantity, box._data));
    ...
    .factory('ngCartItem', ['$rootScope', '$log', function ($rootScope, $log) {
        ...
        var item = function (id, name, price, weight, size, quantity, category, categoryname, categoryquantity, data) {
            this.setId(id);
            this.setName(name);
            this.setPrice(price);
        ...
        item.prototype.setCategoryName = function(categoryname){
            if (categoryname) this._categoryname = categoryname;
        };

        item.prototype.getCategoryName = function(){
            if (this._categoryname) return this._categoryname;
            else $log.info('This item has no categoryname');
        };
    ...
    .factory('ngCartBox', ['$rootScope', '$log', function ($rootScope, $log) {
        ...
        var box = function (id, name, price, weight, size, quantity, category, categoryname, categoryquantity, data) {
            this.setId(id);
            this.setName(name);
            this.setPrice(price);
            this.setWeight(weight);
        ...
        box.prototype.setCategoryName = function(categoryname){
            if (categoryname)  this._categoryname = categoryname;
            else {
                $log.error('A name must be provided');
            }
        };
        box.prototype.getCategoryName = function(){
    ...
    .controller('CartController',['$scope', 'ngCart', '$sce', function($scope, ngCart, $sce) {
        $scope.ngCart = ngCart;
