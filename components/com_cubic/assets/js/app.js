...
myApp.controller ('myCtrl', ['$scope', '$http', 'ngCart', function($scope, $http, ngCart) {
    ...
    $scope.selectcategory= function(index, category) {
       ...
       if(jQuery(window).width() <= 1200 ){
         if(category.id != 2) {
          ...
          jQuery('.bannermoduletop_main').hide();
          jQuery('#TellUsAreaRow').hide();
          jQuery( "#headerProtostar" ).height(0);
         }
       }
    };
    ...
    $scope.selectsubcategory= function(index, subcategory) {
       ...
       if(jQuery(window).width() <= 1200 ){ jQuery('#sideviewtoggle1_start').trigger('click'); jQuery('.bannermoduletop_main').hide(); jQuery('#TellUsAreaRow').hide(); jQuery( "#headerProtostar" ).height(0); }
    };    
    ...
    $scope.emailInventoryPlan = function() {
        ...
        if($scope.ei_email.length > 0){
          ...
          request.done(function( msg ) {
            //$( "#log" ).html( msg );
              //console.log('success!');
              if(jQuery(window).width() <= 1200 ){ jQuery('#sideviewtoggle2_start').trigger('click'); }
