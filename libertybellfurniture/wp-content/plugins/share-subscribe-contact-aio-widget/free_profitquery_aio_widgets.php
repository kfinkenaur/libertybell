...
*/
/**
* Plugin Name: Best Free Increase Traffic & Engage Visitors Widgets
* Plugin URI:http://profitquery.com/?utm=aio_widgets_wp
* Description:Profitquery it is a biggest Free high technology platform to help to each website reach their goals like getting more sore, get more followers, get more sales, get more YouTube views, get more email subscribers, to get more visitors feedbacks, to present visitors special offers, to promote content with social media, to integrate and control external widgets like Facebook messenger, Vimeo video, soundcloudaudio, twitter news feed, telegram chats, social networking Follow Us widgets etc.
* Version:5.2.12
*
* Author:Profitquery Team <support@profitquery.com>
* Author URI:http://profitquery.com/?utm=aio_widgets_wp
*/
...
// HOOK
register_deactivation_hook(__FILE__ ,array('PQ_AIO_Class', 'pluginDeactivation'));
register_activation_hook(__FILE__ ,array('PQ_AIO_Class', 'pluginActivation'));
...
if (!defined('PQ_AIO_ADMIN_IMG_PATH'))
	define('PQ_AIO_ADMIN_IMG_PATH', 'i/');
...
function PQ_AIO_RateUs(){
	...
	.pq_activate{padding:0;margin:15px 0;background-color:#feca16;position:relative;overflow:hidden;background-image:url(<?php echo  plugins_url( PQ_AIO_ADMIN_IMG_PATH.'/banner.png', __FILE__ );?>);background-repeat:no-repeat;background-position:right center;background-size:contain}
	...
	.aa_header {font-size:20px;font-family:arial;margin:25px 0 5px 25px}
		...
		<form id="PQ_AIO_marketing_form" action="<?php echo admin_url("options-general.php?page=" . PQ_AIO_PAGE_NAME);?>" method="POST"> 			
			<div class="pq_activate">  
				
				<div class="aa_button_container" onclick="document.getElementById('PQ_AIO_marketing_form').submit();">  
					<div class="aa_button_border">          
						<div class="aa_button"><?php echo $PQ_AIO_Class->_dictionary[new_message][button];?></div>
					</div>  
				</div>
				<div class="aa_header"><?php echo $PQ_AIO_Class->_dictionary[new_message][title];?></div>
				<div class="aa_description"><?php echo $PQ_AIO_Class->_dictionary[new_message][desc];?></div>  
			</div>  
		</form>  
	</div>	
	<?php
...
function PQ_AIO_RGBA_color($bg, $opacity, $prefix){
	$ret = '';	
	if($bg){		
...
function PQ_AIO_getThankCode($data){
	...
	//designOptions	
	$return['designOptions'] = $data['typeWindow'].' '.$data['popup_form'].' '.PQ_AIO_RGBA_color($data['background_color'], $data['background_opacity'], 'pq_bg_bgcolor_PQRGBA_').' '.$data['head_font'];
	...
	$return['designOptions'] .= ' '.PQ_AIO_getNormalColorStructure('border_color', $data['border_color']).' '.$return['border_depth'].' '.$data['button_font'].' '.PQ_AIO_getNormalColorStructure('button_text_color', $data['button_text_color']).' '.PQ_AIO_getNormalColorStructure('button_color', $data['button_color']);
	...
	$return['designOptions'] .= ' '.$data['ss_view_type'].' '.PQ_AIO_RGBA_color($data['ss_color'], $data['ss_color_opacity'], 'pq_pro_color_socicons_PQRGBA_').' '.PQ_AIO_RGBA_color($data['ss_background_color'], $data['ss_background_color_opacity'], 'pq_pro_bgcolor_socicons_PQRGBA_');
...
function PQ_AIO_prepareCode($data, $bookmark = 0){	
...
/************************************************GENERATE TYPE WINDOW********************************************************************************/
	...
	if((int)$bookmark == 1){
		...
		$return['typeBookmarkWindow'] = ' '.PQ_AIO_getNormalColorStructure('bookmark_background_color', $data['bookmark_background']).' '.PQ_AIO_getNormalColorStructure('bookmark_text_color',$return['bookmark_text_color']).' '.$return['bookmark_text_font'].' '.$return['bookmark_text_size'];
	...
	$return['designOptions'] .=' '.$return['ss_view_type'].' '.PQ_AIO_RGBA_color($data['ss_color'], $data['ss_color_opacity'], 'pq_pro_color_socicons_PQRGBA_').' '.PQ_AIO_RGBA_color($data['ss_background_color'], $data['ss_background_color_opacity'], 'pq_pro_bgcolor_socicons_PQRGBA_');
	...
	$return['designOptions'] .=' '.PQ_AIO_RGBA_color($data['background_color'], $data['background_opacity'], 'pq_bg_bgcolor_PQRGBA_').' '.$return['text_font'].' '.$return['font_size'].' '.PQ_AIO_getNormalColorStructure('text_color',$return['text_color']).' '.$return['button_font'];
	...
	$return['designOptions'] .=' '.PQ_AIO_getNormalColorStructure('border_color',$return['border_color']).' '.$return['border_depth'].' '.$return['popup_form'].' '.$return['head_font'];
	...
	$return['typeDesign'] .= ' '.PQ_AIO_RGBA_color($data['ss_color'], $data['ss_color_opacity'], 'pq_pro_color_socicons_PQRGBA_').' '.PQ_AIO_RGBA_color($data['ss_background_color'], $data['ss_background_color_opacity'], 'pq_pro_bgcolor_socicons_PQRGBA_');
...
function PQ_AIO_code_injection(){
	...
	$sendMailWindow = PQ_AIO_prepareCode($profitquery['sharingSidebar']['sendMailWindow'], 0, 1);	
	...
	$sendMailWindow = PQ_AIO_prepareCode($profitquery['sharingPopup']['sendMailWindow'], 0, 1);
	...
	$sendMailWindow = PQ_AIO_prepareCode($profitquery['sharingBar']['sendMailWindow'], 0, 1);
	...
	$sendMailWindow = PQ_AIO_prepareCode($profitquery['sharingFloating']['sendMailWindow'], 0, 1);
	...
	//ContactFormCenter
	$preparedObject = PQ_AIO_prepareCode($profitquery['contactFormCenter']);	
	//close_icon
	$preparedObject['displayRules']['main_page'] = $profitquery['settings']['mainPage'];	
	$preparedObject['to_email'] = $profitquery['settings']['email'];		
	$PQ_AIO_code['contactFormCenter'] = array(
		'contOptions'=>$preparedObject['contOptions'],
		'designOptions'=>$preparedObject['designOptions'],
		'enable'=>(int)$preparedObject['enable'],		
		'to_email'=>wp_unslash($preparedObject['to_email']),
