...
function check_for_update($checked_data) {
	...
	$raw_response = wp_remote_post(PANORAMIC_UPDATE_URL . '/api/', $send_for_check);
...
function my_theme_api_call($def, $action, $args) {
	...
	$request = wp_remote_post(PANORAMIC_UPDATE_URL . '/api/', $request_string);

	if (is_wp_error($request)) {
		$res = new WP_Error('themes_api_failed', __('An Unexpected HTTP Error occurred during the API request.</p> <p><a href="?" onclick="document.location.reload(); return false;">Try again</a>', 'panoramic'), $request->get_error_message());
	} else {
		$res = unserialize($request['body']);
		
		if ($res === false)
			$res = new WP_Error('themes_api_failed', __('An unknown error occurred', 'panoramic'), $request['body']);
	}
