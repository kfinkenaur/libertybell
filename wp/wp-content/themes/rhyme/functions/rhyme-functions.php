<?php
/**
 * Rhyme functions and definitions
 *
 * @package Rhyme
 *
 *
...
function rhyme_setup() {
	 ...
	 * 
	 */
	load_theme_textdomain( 'rhyme', get_template_directory() . '/languages' );
 	...
 	* Register Navigation
 	*/
	register_nav_menu('main_navigation', __('Primary Menu', 'rhyme') );
...
add_action( 'after_setup_theme', 'rhyme_setup' );

/**
 * Function to change excerpt more string
 */
function rhyme_custom_excerpt_more( $more ) {
	return '...';
}

add_filter( 'excerpt_more', 'rhyme_custom_excerpt_more' );
 ...
 *
 */
function rhyme_wp_title( $title, $sep ) {
	global $paged, $page;
	...
	if ( $paged >= 2 || $page >= 2 )
		$title = "$title $sep " . sprintf( __( 'Page %s', 'rhyme' ), max( $paged, $page ) );

	return $title;
}

add_filter( 'wp_title', 'rhyme_wp_title', 10, 2 );

/** 
 * Register widgetized locations
 */
function rhyme_widgets_init() {
	register_sidebar(array(
		'name' => __( 'Main Sidebar', 'rhyme' ),
...
add_action( 'widgets_init', 'rhyme_widgets_init' );

/** 
 * Add scripts function
 */
function rhyme_add_script_function() {
	/** 
	* Enqueue css
	*/
	wp_enqueue_style('rhyme',  get_stylesheet_uri());
...
add_action('wp_enqueue_scripts','rhyme_add_script_function');

/**
 * Sets up theme defaults CSS rules
 */
function rhyme_custom_styling() {
	/**
	 * General Settings 
	 */		
	...
	$fp_title_color = of_get_option('fp_title_color');
	$fp_title_bg_color = of_get_option('fp_title_bg_color');
	...
	if ( $nav_border_color )
	$output .= '#site-navigation {border-bottom: 4px solid ' . $nav_border_color . '}' . "\n";
	$output .= '#site-navigation {border-top: 4px solid ' . $nav_border_color . '}' . "\n";
	 ...
	 */
	if ($fp_title_color)
	$output .= '.featured-text h3 {color:' . $fp_title_color . '}' . "\n";
	
	if ($fp_title_bg_color)
	$output .= '.featured-text h3 {background: none repeat scroll 0 0 ' . $fp_title_bg_color . '}' . "\n";
...
add_action('wp_head','rhyme_custom_styling');

/** 
 * Load function to change excerpt length
 */
function rhyme_excerpt_length( $length ) {
...
function rhyme_paging_nav() {
	...
	$links = paginate_links( array(
		...
		'prev_text' => __( '&larr; Previous', 'rhyme' ),
		'next_text' => __( 'Next &rarr;', 'rhyme' ),
	) );
...
*/
function rhyme_comment( $comment, $args, $depth ) {
	...
	<li class="post pingback">
		<p><?php __( 'Pingback:', 'rhyme' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( __( 'Edit', 'rhyme' ), '<span class="edit-link">', '</span>' ); ?></p>
	<?php
			break;
		default :
	?>
		...
		<div id="comment-<?php comment_ID(); ?>" class="comment">
			<div class="comment-meta">
				...
				<div class="comment-author-name">
					...
					<?php edit_comment_link( __( 'Edit', 'rhyme' ), '<span class="edit-link">', '</span>' ); ?>
				</div>
				<?php if ( $comment->comment_approved == '0' ) : ?>
					<em class="comment-awaiting-moderation"><?php __( 'Your comment is awaiting moderation.', 'rhyme' ); ?></em>
					<br>
				<?php endif; ?>
			...
			<div>
				<div class="reply">
				<?php comment_reply_link( array_merge( $args, array( 'reply_text' => __( 'Reply <span>&darr;</span>', 'rhyme' ), 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?>
				</div><!--reply-->
			</div>
			</div>
...
*/
function rhyme_add_scrollup() { 
	if ( of_get_option('enable_scrollup') == 1) { 
		echo '<a href="#" class="back-to-top"><i class="fa fa-arrow-circle-up"></i></a>'."\n";
	}
}

add_action('wp_footer', 'rhyme_add_scrollup');

/** 
 * Theme Options Support 
*/
add_action( 'optionsframework_before','rhyme_options_support' );

function rhyme_options_support() { ?>
	<div id="optionsframework-support">
		<div class="metabox-holder">
			<div class="postbox">
					<div class="inside">
                        <p class="btn"><a class="button green" target="_blank" href="http://www.vpthemes.com/support/"><?php _e('Theme Support','rhyme') ?></a> <a class="button orange" target="_blank" href="http://vpthemes.com/preview/Rhyme/"><?php _e('View Demo','rhyme') ?></a> <a class="button blue" target="_blank" href="http://www.vpthemes.com/rhyme/#theme-pricing"><?php  _e('Upgrade to Pro','rhyme') ?></a><strong> If you like my work. Buy me a coffee.</strong></p>
                        <div class="donate">
							<form action="https://www.paypal.com/cgi-bin/webscr" method="post" target="_blank">
								<input type="hidden" name="cmd" value="_s-xclick">
								<input type="hidden" name="hosted_button_id" value="AJTD32E3UWSSY">
...
function rhyme_content_boxes() { ?>
	<div class="content-boxes">
		<div class="col">
			...
			<a class="content-btn" href="<?php echo esc_url(of_get_option('first_column_url')); ?>">Read More</a>
		</div>
		<div class="col">
			...
			<a class="content-btn" href="<?php echo esc_url(of_get_option('second_column_url')); ?>">Read More</a>
		</div>
		<div class="col">
			...
			<a class="content-btn" href="<?php echo esc_url(of_get_option('third_column_url')); ?>">Read More</a>
		</div>
	</div>
<?php }
...
*/
function rhyme_the_attached_image() {
	$post = get_post();
	$attachment_size = apply_filters( 'rhyme_attachment_size', array( 1024, 1024 ) );
