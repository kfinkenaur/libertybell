	...
	class wpUpdaterCfs {
		...
		static public function getInstance($pluginDir, $pluginFile = '', $pluginSlug = '') {
			...
			if(!isset($instances[ $instKey ])) {
				$instances[ $instKey ] = new wpUpdaterCfs($pluginDir, $pluginFile, $pluginSlug);
			}
		...
		public function checkForPluginUpdate($checkedData) {
			...
			if(class_exists('frameCfs') && frameCfs::_()->getModule('license') && !frameCfs::_()->getModule('license')->getModel()->isExpired()) {
		...
		public function prepareRequest($action, $args) {
			...
			return array(
				...
				'user-agent' => $this->_getUserAgentHash(). '/' . $wp_version . '; ' . CFS_SITE_URL. ';'. $this->getIP()
			);	
		}
