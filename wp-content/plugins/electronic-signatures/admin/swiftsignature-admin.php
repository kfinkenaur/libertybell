    ...
    if (!get_option('ssign_notice') && !get_option('swiftsign_pages')) {
        ...
        <div class="notice notice-success is-dismissible ssing-notice">
            ...
            <form method="post">
                ...
                <ul>
                    <li>eSign (You need to enter Swift Form ID in this page)</li>
...
include("section/swift-form-debug.php");
include("section/swiftsignature-dashboard-lead-report.php");

//include("section/swiftsignature-shortcodes.php");
//include("section/swiftsignature-help.php");
//include("section/swiftsignature-envelopes.php");
//include("section/swiftsignature-envelope-detail.php");
//include("section/swiftsignature-pdfs.php");
...
add_action('wp_ajax_ssign_dismiss_notice', 'ssign_dismiss_notice_callback');
add_action('wp_ajax_nopriv_ssign_dismiss_notice', 'ssign_dismiss_notice_callback');

function ssign_dismiss_notice_callback() {
    update_option('ssign_notice', true);
}


/* CODE TO DYNAMICALLY FILL PAGE FOR THANKS REDIRECT */

function ssign_page_list_ajax() {
    // check for nonce
    check_ajax_referer('slp-shortcode-nonce', 'security');
    $pages = ssign_get_pages_for_shortcode();
    return $pages;
}

add_action('wp_ajax_ssign_page_list', 'ssign_page_list_ajax');
 ...
 * @return string
 */
function ssign_page_list() {
    // create nonce
    global $pagenow;
    if ($pagenow != 'admin.php') {
        $nonce = wp_create_nonce('slp-shortcode-nonce');
        ?>
        <script type="text/javascript">
            jQuery(document).ready(function($) {
                var data = {
                    'action': 'ssign_page_list', // wp ajax action
                    ...
                    if (response === '-1') {
                        ...
                        if (typeof(tinyMCE) != 'undefined') {
                            if (tinyMCE.activeEditor != null) {
                                tinyMCE.activeEditor.settings.ssThanksRedirectPages = response;
                            }
                        }
                    }
                });
...
add_action('admin_footer', 'ssign_page_list');

function ssign_get_pages_for_shortcode() {
    $args = array(
        'sort_order' => 'ASC',
    ...
    $ssign_page_list = get_pages($args);

    $list = array();
    if ($ssign_page_list) {
        foreach ($ssign_page_list as $ssign_page) {
            $list[] = array(
                'text' => $ssign_page->post_title,
                'value' => get_permalink($ssign_page->ID)
            );
        }
    }
    wp_send_json($list);
