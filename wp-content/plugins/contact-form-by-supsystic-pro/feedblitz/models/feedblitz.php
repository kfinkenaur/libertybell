<?php
class feedblitzModelCfs extends modelSubscribeCfs {
	public function getLists() {
		...
		if($listsRes && is_array($listsRes)) {
			...
			$this->pushError(__('You have no lists. Login to your FeedBlitz account and create your first list before start using this functionality.', CFS_LANG_CODE));
	...
	public function makeRequest($address, $data = array()) {
		...
		if(empty($response)) {
			...
			$res = utilsCfs::jsonDecode($response);
		}
		if(!empty($res['error'])) {
	...
	public function subscribe($d, $form, $validateIp = false) {
		...
		if(!empty($email)) {
			if(is_email($email)) {
				$lists = isset($form['params']['tpl']['sub_sg_lists']) ? $form['params']['tpl']['sub_sg_lists'] : array();
				if(!empty($lists)) {
					if($this->isLoggedIn()) {
