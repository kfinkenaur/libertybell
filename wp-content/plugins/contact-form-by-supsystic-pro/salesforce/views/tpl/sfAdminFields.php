...
<tr class="cfsFormSubDestOpts cfsFormSubDestOpts_salesforce">
	<th scope="row">
...
<tr class="cfsFormSubDestOpts cfsFormSubDestOpts_salesforce">
	<th scope="row">
		<?php _e('Form ID', CFS_LANG_CODE)?>
		<i class="fa fa-question supsystic-tooltip-bottom" title="<?php echo esc_html(sprintf(__('Generate your lead form in your Salesforce account (more about generating form you can read <a target="_blank" href="%s">here</a>), then copy "oid" value from it (<a target="_blank" href="%s">like this</a>) and insert it into this paramter', CFS_LANG_CODE), 'https://help.salesforce.com/apex/HTViewHelpDoc?id=setting_up_web-to-lead.htm&language=en', $this->helpImgUrl))?>"></i>
	</th>
	<td>
		<?php echo htmlCfs::text('params[tpl][sub_sf_app_id]', array(
			'value' => (isset($this->form['params']['tpl']['sub_sf_app_id']) ? $this->form['params']['tpl']['sub_sf_app_id'] : ''),
		))?>
	</td>
</tr>
<?php }?>
