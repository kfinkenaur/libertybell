<?php
class verticalresponseModelCfs extends modelSubscribeCfs {
	...
	public function saveAccessToken($d = array()) {
		if(isset($d['code']) && !empty($d['code'])) {
			...
			$authRes = $this->makeRequest('oauth/access_token?client_id='. $this->getClientId(). 
				...
				'&redirect_uri='. $this->getFullRedirectUri($formId). '&code='. $d['code']);
			if($authRes && is_array($authRes)) {
				frameCfs::_()->getModule('options')->getModel()->save($this->getCode(). '_access_token', $authRes['access_token']);
				return true;
			} else
	...
	public function getFullRedirectUri($formId = 0) {
		return admin_url(). $this->getModule()->getRedirectAlias(). '/'. $formId;
	}
	...
	public function subscribe($d, $form, $validateIp = false) {
		...
		if(!empty($email)) {
			if(is_email($email)) {
				$lists = isset($form['params']['tpl']['sub_vr_lists']) ? $form['params']['tpl']['sub_vr_lists'] : array();
				if(!empty($lists)) {
					if($this->isLoggedIn()) {
