...
class constantcontactModelCfs extends modelSubscribeCfs {
	...
	public function getLists() {
		...
		if($ccObj) {
			if($this->isLoggedIn()) {
				...
				$this->pushError(langCfs::_('You are not logged-in'));
		} else
			$this->pushError(langCfs::_('Can not get cc obj'));
	...
	public function isLoggedIn() {
		return (!frameCfs::_()->getModule('options')->isEmpty($this->getCode(). '_access_token'));
	}
	public function saveAccessToken($d = array()) {
		if(isset($d['error']) && !empty($d['error'])) {
			...
			try {
				...
				$accessTokenData = $this->getAuthObj( $formId )->getAccessToken($d['code']);
				frameCfs::_()->getModule('options')->getModel()->save($this->getCode(). '_access_token', $accessTokenData['access_token']);
				return true;
	...
	public function getAuthObj($formId = 0) {
		if(!isset($this->_authObjList[ $formId ])) {
			$this->_authObjList[ $formId ] = new CtctOAuth2(
				$this->_appKey, 
				$this->_appSecret,
				$this->_redirectUri. '?site='. admin_url(). '&pl='. CFS_CODE. '&id='. $formId
			);
		}
		return $this->_authObjList[ $formId ];
	}
	...
	public function subscribe($d, $form, $validateIp = false) {
		...
		if(!empty($email)) {
			if(is_email($email)) {
				$lists = isset($form['params']['tpl']['sub_cc_lists']) ? $form['params']['tpl']['sub_cc_lists'] : array();
				if(!empty($lists)) {
					if($this->isLoggedIn()) {
