<?php
class sendinblueModelCfs extends modelSubscribeCfs {
		...
		if(!empty($apiKey)) {
			...
			if($listsRes && $listsRes['code'] == 'success') {
				if(isset($listsRes['data']) && !empty($listsRes['data'])) {
					...
					$this->pushError(__('You have no list for now. Create them at first in your SendingBlue account', CFS_LANG_CODE));
			} elseif(isset($listsRes['message']) && !empty($listsRes['message'])) {
				$this->pushError($listsRes['message']);
			} else {
				$this->pushError(__('Can not get Lists', CFS_LANG_CODE));
			}
			return false;
		} else
			$this->pushError(__('Please enter your API key', CFS_LANG_CODE), 'params[tpl][sub_sb_lists]');
		return false;
	...
	public function subscribe($d, $form, $validateIp = false) {
		...
		if(!empty($email)) {
			if(is_email($email)) {
				$lists = isset($form['params']['tpl']['sub_sb_lists']) ? $form['params']['tpl']['sub_sb_lists'] : array();
				if(!empty($lists)) {
					if(!$validateIp || $validateIp && frameCfs::_()->getModule('subscribe')->getModel()->checkOftenAccess(array('only_check' => true))) {
						...
						$apiKey = $form['params']['tpl']['sub_sb_api_key'];
