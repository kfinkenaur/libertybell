<?php
class benchmarkemailModelCfs extends modelSubscribeCfs {
			...
			if(!empty($password)) {
				...
				if($client) {
					...
					if(!empty($listsRes) && count($listsRes) > 0) {
						...
						$this->pushError(__('Please make sure that you created some Lists under your Benchmark account.', CFS_LANG_CODE));
					}
				}
			} else 
				$this->pushError(__('Please enter your Benchmark Password', CFS_LANG_CODE), 'sub_be_pass');
		} else
			$this->pushError(__('Please enter your Benchmark Login', CFS_LANG_CODE), 'sub_be_login');
		return false;
	...
	public function subscribe($d, $form, $validateIp = false) {
		...
		if(!empty($email)) {
			if(is_email($email)) {
				$lists = isset($form['params']['tpl']['sub_be_lists']) ? $form['params']['tpl']['sub_be_lists'] : array();
				if(!empty($lists)) {
	...
	public function getApiUsername() {
		if(empty($this->_username)) {
			$this->_username = $this->_decrypt( frameCfs::_()->getModule('options')->get($this->getCode(). '_username') );
		}
		return $this->_username;
	}
	public function getApiPassword() {
		if(empty($this->_password)) {
			$this->_password = $this->_decrypt( frameCfs::_()->getModule('options')->get($this->getCode(). '_password') );
		}
		return $this->_password;
	}
	public function setApiUsername($username) {
		frameCfs::_()->getModule('options')->getModel()->save($this->getCode(). '_username', $this->_encrypt($username));
	}
	public function setApiPassword($password) {
		frameCfs::_()->getModule('options')->getModel()->save($this->getCode(). '_password', $this->_encrypt($password));
	}
}