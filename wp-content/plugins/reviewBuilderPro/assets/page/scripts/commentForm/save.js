function SGRB(){
	'use strict';
	this.init();
	SGRB.inputTypeText = 1;
	SGRB.inputTypeEmail = 2;
	SGRB.inputTypeNumber = 3;
	/*SGRB.inputTypePassword = 4;*/
	SGRB.inputTypeTextarea = 5;
};

SGRB.prototype.init = function() {
	var that = this;

	if (SGRB.prototype.getURLParameter('edit')) {
...
SGRB.prototype.deleteField = function(field) {
	var index = 1;
	var deletedField = '';
	if (confirm('Are you sure?')) {
		...
		jQuery('.sgrb-form-row').each(function(){
			...
			jQuery(this).find('.sgrb-delete-form-field-js').attr('onclick', 'SGRB.prototype.deleteField('+index+')');
...
SGRB.ajaxCloseBanner = function(){
	var deleteAction = 'CommentForm_ajaxCloseBanner';
 ...
 */
SGRB.prototype.getURLParameter = function(params) {
...
SGRB.prototype.insertFieldShortcode = function() {
	var index = 1;
	...
	jQuery('.sgrb-attributes-input').each(function(){
		...
		SGRB.prototype.applyInputValue(inputName);
	});
	...
	if (createdFormTag) {
		var textareaHtml = '<div id="sgrb-field-'+index+'" class="sg-row sgrb-form-row">'+
										...
										'<div class="sg-col-1 sgrb-minus-icon-wrapper">'+
											'<img onclick="SGRB.prototype.deleteField('+index+');" class="sgrb-form-minus-icon sgrb-delete-form-field-js" src="'+sgrbAppUrl+'assets/page/img/remove_image.png">'+
										'</div>'+
									'</div>';
...
SGRB.prototype.applyInputValue = function(inputName) {
	var shortcodeToEdit = jQuery('.sgrb-created-text-tag').val();
...
SGRB.prototype.selectUseAs = function(valueToApply) {
	var shortcodeToEdit = jQuery('.sgrb-created-text-tag').val();
