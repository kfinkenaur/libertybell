...
class SupsysticSliderPro_Photos_vendors_vimeo_api_Vimeo
{
    ...
    public function __construct($client_id, $client_secret, $access_token = null)
    {
        ...
        $this->CURL_DEFAULTS = array(
            ...
            CURLOPT_CAINFO => realpath(__DIR__) . DIRECTORY_SEPARATOR . 'certificates' . DIRECTORY_SEPARATOR . 'vimeo-api.pem'
        );
    }
    ...
    public function uploadImage($pictures_uri, $file_path, $activate = false)
    {
        ...
        if ($pictures_response['status'] !== 201) {
            throw new \Exception('Unable to request an upload url from vimeo');
        }
    ...
    private function _request($url, $curl_opts = array())
    {
        ...
        if (isset($curl_info['http_code']) && $curl_info['http_code'] === 0) {
            ...
            throw new \Exception('Unable to complete request.' . $curl_error);
        }
    ...
    private function perform_upload($file_path, $ticket)
    {
        if ($ticket['status'] !== 201) {
            ...
            throw new \Exception('Unable to get an upload ticket.' . $ticket_error);
        }
        ...
        if ($status !== 201) {
            ...
            throw new \Exception('Error completing the upload.'. $error);
        }
