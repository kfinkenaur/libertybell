					...
					$this->getText(0, 'copyright')
                        ...
                        foreach($this->forms As $form_id){
                            ...
                            if(trim($row->template_code) != ''){
                                $areas = Zend_Json::decode(BFBase::decode($row->template_areas));
                                $i = 0;
                                ...
                                if ($row->template_code_processed == 'QuickMode') {
                                    $dataObject = Zend_Json::decode(BFBase::decode($row->template_code));
                                    $this->resetQuickModeDbId($dataObject);
                                    $template_code = BFBase::encode(Zend_Json::encode($dataObject));
                                }
                                ...
                                if($row && $row->template_code_processed == 'QuickMode'){
                                    ...
                                    $quickMode->save(
                                        $form_id,
                                        Zend_Json::decode( BFBase::decode( $template_code ))
                                    );
                                }
                            }
                        }
	...
	function emitForm()
	{
		...
		if (!array_key_exists('emitted', $this->params[1])) {
			...
			if($this->getText(1, 'template_code') != ''){
				$row->template_code = BFBase::decode($this->getText(1, 'template_code'));
				$row->template_code_processed = BFBase::decode($this->getText(1, 'template_code_processed'));
