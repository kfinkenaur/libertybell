	...
	function mkPackage($option, $caller, $pkg)
	{
		...
		if (count($ids) > 0) {
			...
			for ($f = 0; $f < count($forms); $f++) {
				...
				if($form->template_code != '')$xml.=indent(2).'<template_code>'.BFBase::encode($form->template_code).'</template_code>';
				if($form->template_code_processed != '')$xml.=indent(2).'<template_code_processed>'.BFBase::encode($form->template_code_processed).'</template_code_processed>';
				if($form->template_areas != '')$xml.=indent(2).'<template_areas>'.BFBase::encode($form->template_areas).'</template_areas>';
