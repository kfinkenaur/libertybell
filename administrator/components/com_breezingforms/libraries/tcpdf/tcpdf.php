...
class TCPDF {
	...
	public function Output($name='doc.pdf', $dest='I') {
		...
		if ($this->sign) {
			...
			unset($tmparr);
			// decode signature
			$signature = BFBase::decode(trim($signature));
		...
		switch($dest) {
			...
			case 'E': {
                                $sixtyfour = 64;
				// return PDF as  mime multi-part email attachment (RFC 2045)
				$retval = 'Content-Type: application/pdf;'."\r\n";
				$retval .= ' name="'.$name.'"'."\r\n";
				$retval .= 'Content-Transfer-Encoding: base'.$sixtyfour."\r\n";
				$retval .= 'Content-Disposition: attachment;'."\r\n";
				$retval .= ' filename="'.$name.'"'."\r\n\r\n";
				$retval .= chunk_split(BFBase::encode($this->getBuffer()), 76, "\r\n");
				return $retval;
			}
			case 'S': {
	...
	protected function _generateencryptionkey() {
		...
		if (!$this->encryptdata['pubkey']) { // standard mode
			...
			foreach ($this->encryptdata['pubkeys'] as $pubkey) {
				...
				unset($tmparr);
				// decode signature
				$signature = BFBase::decode($signature);
				// convert signature to hex
			...
			case 'img': {
				if (!empty($tag['attribute']['src'])) {
					if ($tag['attribute']['src']{0} === '@') {
						// data stream
						$tag['attribute']['src'] = '@'.BFBase::decode(substr($tag['attribute']['src'], 1));
						$type = '';
					} else {
						// get image type
		...
		switch($name) {
			...
			case 'image': {
				...
				if (!$clipping) {
					...
					if (preg_match('/^data:image\/[^;]+;base'.$sixtyfour.',/', $img, $m) > 0) {
						// embedded image encoded as 
						$img = '@'.BFBase::decode(substr($img, strlen($m[0])));
					} else {
						// fix image path
